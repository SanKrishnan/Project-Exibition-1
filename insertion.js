...
Time Complexity: O(N^2)`;
    ...
    space.innerHTML = `
The space complexity of insertion sort is <b>O (1)</b>.
...
async function InsertionSort() {
    ...
    for (let i = 1; i < element.length; i++) {
        ...
        element[i].style.background = 'rgb(0, 5, 54)';
        await waitforme(delay);

        while (j >= 0 && (parseInt(element[j].style.height) > parseInt(p))) {
            element[j].style.background = 'rgb(0, 102, 2)';
            ...
            for (let k = i; k >= 0; k--) {
                element[k].style.background = 'rgb(0, 252, 11)';
            }
        }
        element[j + 1].style.height = p;
        element[i].style.background = 'rgb(0, 252, 11)';
    }

    // Number the sorted bars
    for (let i = 0; i < element.length; i++) {
        element[i].textContent = i + 1; // Assign ranks from 1 to n
    }
