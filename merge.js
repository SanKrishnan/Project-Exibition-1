...
async function Merge(element, low, mid, high) {
   ...
   // Copy data to temp arrays and change color for visualization
   for (let i = 0; i < a1; i++) {
       await waitforme(delay);
       beep.play();
       element[low + i].style.background = 'black';
       left[i] = parseInt(element[low + i].style.height);
       element[low + i].innerHTML = `<div style="position:relative;top:-20px;color:black;">${left[i]}</div>`; // Ensure numbers are always visible
   }

   for (let i = 0; i < a2; i++) {
       await waitforme(delay);
       beep.play();
       element[mid + 1 + i].style.background = 'beige';
       right[i] = parseInt(element[mid + 1 + i].style.height);
       element[mid + 1 + i].innerHTML = `<div style="position:relative;top:-20px;color:black;">${right[i]}</div>`; // Ensure numbers are always visible
   }
   await waitforme(delay);
   ...
   while (i < a1 && j < a2) {
       ...
       beep.play();
       if (left[i] <= right[j]) {
           element[k].style.height = left[i] + 'px';
           element[k].innerHTML = `<div style="position:relative;top:-20px;color:black;">${left[i]}</div>`; // Ensure numbers are always visible
           element[k].style.background = 'lightgreen';
           i++;
       } else {
           element[k].style.height = right[j] + 'px';
           element[k].innerHTML = `<div style="position:relative;top:-20px;color:black;">${right[j]}</div>`; // Ensure numbers are always visible
           element[k].style.background = 'lightgreen';
           j++;
       }
       k++;
   ...
   for ( let m = low; m <= high; m++) {
       await waitforme(delay);
       element[m].style.background = 'rgb(0, 255, 0)'; // Set to green after sorting
       const height = parseInt(element[m].style.height);
       element[m].innerHTML = `<div style="position:relative;top:-20px;color:black;">${height}</div>`; // Ensure numbers are always visible
   }
}
