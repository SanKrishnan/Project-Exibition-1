...
QuickSortbutton.addEventListener('click', async function () {
    ...
    for (let i = 0; i < element.length; i++) {
        element[i].textContent = i + 1; // Assign rank based on sorted order
    }
...
`
    ...
    time.innerHTML = 
`Worst Case: The worst case occurs when the partition process always picks the greatest or smallest element as the pivot.
...
async function partition(element, low, high) {
    beep.play();
    let i = low - 1;
    element[high].style.background = 'black';
    for (let j = low; j <= high - 1; j++) {
        beep.play();
        element[j].style.background = 'lime';
        ...
        if (parseInt(element[j].style.height) < parseInt(element[high].style.height)) {
            ...
            element[i].style.background = 'teal';
            if (i != j) element[j].style.background = 'teal';
    ...
    element[high].style.background = 'beige';
